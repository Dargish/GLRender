...
namespace graphics
{
	namespace lights
	{
		DirectionalLight::DirectionalLight() :
			m_screenQuad(new Plane), m_direction(Vector3(0, -1, 0)), m_color(Vector3(1, 1, 1)), m_intensity(1.0f)
		{

		}

		DirectionalLight::DirectionalLight(const Vector3& direction, const Vector3& color, float intensity) :
			m_screenQuad(new Plane), m_direction(direction), m_color(color), m_intensity(intensity)
		{

		}

		DirectionalLight::DirectionalLight(const DirectionalLight& other) :
			m_screenQuad(new Plane), m_direction(other.m_direction), m_color(other.m_color), m_intensity(other.m_intensity)
		{
